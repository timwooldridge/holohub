...
// FIXME: add camera extrinsics
struct CameraMessageParts {
  // Message entity
  Entity entity;
  // Camera identifier
  Handle<uint64_t> camera_uid;
  // Camera frame
  Handle<VideoBuffer> frame;
  // Camera intrinsics
  Handle<CameraModel> intrinsics;
  // Frame ID
  Handle<int64_t> frame_number;
...
template <VideoFormat Format>
Expected<CameraMessageParts> CreateCameraMessage(gxf_context_t context,
                                                 uint32_t width,
                                                 uint32_t height,
...
Expected<CameraMessageParts> GetCameraMessage(const Entity message);
