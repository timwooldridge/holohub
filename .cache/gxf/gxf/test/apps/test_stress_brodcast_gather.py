...
def create_greedy_app(n):
  return HEADER + "".join([create_brodcast_gather(i, 1000) for i in range(n)]) + GREEDY_FOOTER

def create_multithread_app(n):
  return HEADER + "".join([create_brodcast_gather(i, 1000) for i in range(n)]) + MULTITHREAD_FOOTER
...
if __name__ == "__main__":
  out_dir = sys.argv[1]
  save(out_dir + '/test_stress_brodcast_gather_greedy_gen.yaml', create_greedy_app(100))
  save(out_dir + '/test_stress_brodcast_gather_multithread_gen.yaml', create_multithread_app(1))
