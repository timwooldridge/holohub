...
class StepCount : public Codelet {
 public:
  ...
  gxf_result_t deinitialize() override {
    if (use_assert_) {
      ...
      if (count_start_ != expected_start_count_.get()) {
        GXF_LOG_ERROR("Start count [%zu] does not match expected start count [%zu]",
                      count_start_, expected_start_count_.get());
        return GXF_FAILURE;
      }
      if (count_tick_ != expected_count_.get()) {
        GXF_LOG_ERROR("Tick count [%zu] does not match expected tick count [%zu]",
                      count_tick_, expected_count_.get());
        return GXF_FAILURE;
      }
  ...
  gxf_result_t registerInterface(Registrar* registrar) override {
    ...
    result &= registrar->parameter(expected_count_, "expected_count", "", "", 0UL);
    return ToResultCode(result);
  }
 ...
 public:
  ...
  gxf_result_t tick() override {
    ...
    for (const auto tensor : tensors.value()) {
      ...
      std::printf("shape: (");
      for (size_t i = 0; i < tensor.value()->rank(); i++) {
        std::printf("%d", tensor.value()->shape().dimension(i));
        if (i + 1 < tensor.value()->rank()) std::printf(", ");
      }
      std::printf("), %s: ", tensor->name());

      PrintSingleTensorAllTypes(*tensor.value());
    }
...
class TestConfigurationSet: public Codelet {
 public:
  gxf_result_t start() override {
    return GXF_SUCCESS;
  }
  ...
  gxf_result_t tick() override {
    GXF_LOG_INFO("TestConfigurationSet ticking:");
    GXF_LOG_INFO("bool_value: %s", bool_value_.get()? "true":"false");
    GXF_LOG_INFO("uint64_value: %llu", uint64_value_.get());
    GXF_LOG_INFO("string_value: %s", string_value_.get().c_str());
    GXF_LOG_INFO("string_5_value: %s", string_5_value_.get().c_str());
    GXF_LOG_INFO("int_vector_4_value: [%d, %d, %d, %d]",
                  int_vector_4_value_.get()[0].value(),
                  int_vector_4_value_.get()[1].value(),
                  int_vector_4_value_.get()[2].value(),
                  int_vector_4_value_.get()[3].value());
    return GXF_SUCCESS;
  }
  ...
  gxf_result_t registerInterface(Registrar* registrar) override {
    Expected<void> result;
    result &= registrar->parameter(bool_value_, "bool_value", "Bool type value for configuration",
                                   "", false);
    result &= registrar->parameter(uint64_value_, "uint64_value", "unsigned int64 for "
                                   "configuration", "");
    result &= registrar->parameter(string_value_, "string_value", "string value for "
                                   "configuration", "");
    result &= registrar->parameter(string_5_value_, "string_5_value", "fixed string value with "
                                   " maximum length of 5", "");
    result &= registrar->parameter(int_vector_4_value_, "int_vector_4_value", "fixed vector of "
                                   "integer value with maximum length of 4", "");

    return ToResultCode(result);
  }

 private:
  Parameter<bool> bool_value_;
  Parameter<uint64_t> uint64_value_;
  Parameter<std::string> string_value_;
  Parameter<FixedString<5>> string_5_value_;
  Parameter<FixedVector<int, 4>> int_vector_4_value_;
};

// Scheduling term that is always in WAIT state
class WaitSchedulingTerm : public SchedulingTerm {
 public:
  gxf_result_t check_abi(int64_t timestamp, SchedulingConditionType* type,
                         int64_t* target_timestamp) const override {
  ...
  gxf_result_t onExecute_abi(int64_t dt) { return GXF_SUCCESS; }
};
